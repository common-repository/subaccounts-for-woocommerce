...
function sfwc_frontend_manage_subaccounts() {
    ...
    if ( 
		...
		if ( $user_account_level_type == 'supervisor' ) {
			...
			if ( isset( $_POST['ajax_filter_account_type'] ) && $_POST['ajax_filter_account_type'] == 'subaccounts' ) {
		...
		if ( $user_account_level_type == 'supervisor' ) { // Logged in user is a Supervisor.
			...
			if ( $filter_account_type == 'subaccounts' ) {
		...
		if ( is_user_logged_in() && ( $user_account_level_type == 'manager' || $user_account_level_type == 'supervisor' ) ) {
			...
			if ( sfwc_is_current_user_role_valid() && sfwc_is_current_user_role_enabled() ) {
				...
				if ( $user_account_level_type == 'supervisor' ) {
					...
					echo '<a id="sfwc_frontend_filter_managers" class="sfwc_frontend_manage_subaccounts_filter ' . $status_managers . '" href="#">';
					esc_html_e( 'Managers', 'subaccounts-for-woocommerce' );
					echo '</a>';
					
					echo '<a id="sfwc_frontend_filter_subaccounts" class="sfwc_frontend_manage_subaccounts_filter ' . $status_subaccounts . '" href="#">';
					...
					<table id="sfwc_frontend_manage_subaccounts_table" class="sfwc_table">
						<thead>
							<tr>
								...
								<?php if ( $user_account_level_type == 'supervisor' && $filter_account_type == 'subaccounts' ): ?>
						...
						foreach ( $user_query_subaccounts->get_results() as $user ) {
							...
							if ( $user_account_level_type == 'supervisor' && $filter_account_type = 'subaccounts'  ) {
		...
		jQuery(document).ready(function ($) {
			...
			var filter_account_type = "<?php echo ( isset( $filter_account_type ) ) ? $filter_account_type : ''; ?>";
...
function sfwc_frontend_edit_subaccount() {
    ...
    if ( wp_verify_nonce( $_POST['nonce'], 'sfwc_nonce_frontend_manage_subaccounts_pagination' ) || wp_verify_nonce( $_POST['nonce'], 'sfwc_nonce_frontend_manage_subaccounts' ) ) {
		...
		// return to previously selected subaccount list (either "Managers" or "Subaccounts").
		if ( $subaccount_account_level_type !== 'manager' && $subaccount_account_level_type !== 'supervisor' ) {
	...
	<?php endif; ?>
		...
		jQuery(document).ready(function ($) {
			...
			var filter_account_type = "<?php echo $filter_account_type; ?>";
