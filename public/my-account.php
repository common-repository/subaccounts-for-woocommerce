...
function sfwc_add_custom_order_number_column( $order ) {
	...
	if ( is_wc_endpoint_url( 'subaccounts' ) ) {
		...
		echo '<a href="' . esc_url( $order_number_url ) . '">' . esc_html( _x( '#', 'hash before order number', 'woocommerce' ) ) . $order->get_order_number() . '</a>';
	}
}
...
function sfwc_add_subaccount_form_handler() {
	...
	if ( ( isset( $user_login ) && $user_login != "" && validate_username( $user_login ) ) && ( isset( $email ) && $email != "" && is_email( $email ) ) ) {
		...
		if ( ! isset( $_POST['sfwc_add_subaccount_frontend'] ) || isset( $_POST['sfwc_add_subaccount_frontend'] ) && ! wp_verify_nonce( $_POST['sfwc_add_subaccount_frontend'], 'sfwc_add_subaccount_frontend_action' ) ) {
			...
			$default_user_role_capabilities = get_role( $default_user_role )->capabilities;
			
			if ( ! empty( $default_user_role_capabilities ) ) {
				
				$result = array_intersect_key( $default_user_role_capabilities, $all_potential_wp_capabilities );
				
				if (
					// Loose Method.
					( defined( 'SFWC_VALID_ROLE_LOOSE_MODE' ) && SFWC_VALID_ROLE_LOOSE_MODE == true && ( array_key_exists( 'read', $default_user_role_capabilities ) && ! array_key_exists( 'edit_posts', $default_user_role_capabilities ) ) )
					// Strict Method.
					|| ( ! empty( $result ) && count( $result ) === 1 && key( $result ) == 'read' )
				) {
						...
						<select id="sfwc-select-subaccount-orders" name="sfwc_subaccount_orders[]" multiple placeholder="<?php esc_attr_e( 'Select account...', 'subaccounts-for-woocommerce' ); ?>">
							...
							if ( ! empty( $existing_children_ids ) ) {
								...
								foreach ( $existing_children_ids as $key => $value ) {
									...
									// Keep selected options in the multi-select field.
				...
				Selectize.define("select_remove_all_options", function(options) {
					...
					self.setup = (function() {
						...
						return function() {
							...
							var allBtn = $("<button type=\"button\" class=\"btn btn-xs btn-success\"><?php esc_html_e( 'Select All', 'subaccounts-for-woocommerce' ); ?></button>");
							var clearBtn = $("<button type=\"button\" class=\"btn btn-xs btn-default\"><?php esc_html_e( 'Clear', 'subaccounts-for-woocommerce' ); ?></button>");
...
function sfwc_set_selected_subaccount_orders_cookie() {
	...
	if ( isset( $_POST['sfwc_subaccount_orders'] ) ) {
		...
		setcookie( apply_filters( 'sfwc_cookie_prefix', '' ) . 'subaccount_ids', json_encode( $selected_subaccount_ids_sanitized ), [
			'expires' => '',
			'path' => COOKIEPATH,
			'domain' => COOKIE_DOMAIN,
			'secure' => is_ssl(),
